...
## 데이터 수 만큼 bikes 정보 출력
for (let i = 0; i < bikes.length; i++) {
    console.log(bikes[i])
}

## 역 이름[stationName] , 따릉이 정류장에 주차된 자전거의 개수[parkingBikeTotCnt]
## i번째 역 이름과 i번째의 주차된 자전거 수를 출력
## 변수의 이름과 데이터를 지정하여 let으로 먼저 선언한 후,
## console.log로 출력할 것
...
// . . .
// 209. 유진투자증권빌딩 앞 12
// VM5290:4 210. IFC몰 16
// VM5290:4 211. 여의도역 4번출구 옆 2
// VM5290:4 212. 여의도역 1번출구 옆 9

// 주차된 자전거 수가 5개보다 적은 장소 출력
for (let i = 0; i < bikes.length; i++) {
    let name = bikes[i]['stationName']
    let bike = bikes[i]['parkingBikeTotCnt']   
    if (bike < 5) {
